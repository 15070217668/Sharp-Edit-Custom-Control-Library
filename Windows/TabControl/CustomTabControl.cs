		...
		public Rectangle GetTabCloserRect(int index){
			...
			using (GraphicsPath path = this._StyleProvider.GetTabBorder(index)){
				...
				if (this.Alignment <= TabAlignment.Bottom) {
					...
					if (this.RightToLeftLayout){
						closerRect = new Rectangle((int)rect.X + (int)Math.Floor((double)((int)rect.Width - 10) /2), (int)rect.Top, 6, 6);
						while (!path.IsVisible(closerRect.X, closerRect.Top) && closerRect.Bottom < this.Height) {
							closerRect.Y += 1;	
						}
						closerRect.Y += 4;
					} else {
						closerRect = new Rectangle((int)rect.X + (int)Math.Floor((double)((int)rect.Width - 10)/2), (int)rect.Bottom, 6, 6);
